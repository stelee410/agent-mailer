...
from agent_mailer_cli.config import Config, VALID_RUNTIMES, save_config

DEFAULT_BROKER_URL = "http://localhost:9800"
DEFAULT_CREDENTIALS_PATH = Path.home() / ".agent-mailer" / "credentials.json"
DEFAULT_PERMISSION_MODE = "acceptEdits"
DEFAULT_SHORTCUT_PERMISSION_MODE = "bypassPermissions"
...
def _write_project_link(agent_dir: Path, project_dir: Path | None) -> None:
    if project_dir is None:
        return
    target = project_dir.expanduser().resolve()
    link = agent_dir / "project"
...
def create_default_team(
    *,
    ...
    resolved_project_dir = project_dir.expanduser().resolve() if project_dir is not None else None
    ...
    for spec in DEFAULT_TEAM_AGENTS:
        ...
        _write_project_link(agent_dir, resolved_project_dir)
        save_config(
            Config(
                workdir=agent_dir,
                agent_id=agent["id"],
    ...
    (out_dir / "team.yaml").write_text(
        render_team_yaml(team, broker_url, permission_mode, runtime, resolved_project_dir)
    )
...
def _initialize_team(
    *,
    ...
    resolved_project_dir = (project_dir or Path.cwd()).expanduser().resolve()
...
def _print_init_summary(
    *,
    ...
    if project_dir is not None:
        click.echo(f"OK Project: {project_dir}")
    for agent in agents:
...
@click.option("--runtime", type=click.Choice(VALID_RUNTIMES), default="claude", show_default=True,
              help="Local execution runtime for each agent.")
@click.option("--credentials-path", default=None, hidden=True)
def init(name: str | None, team: str | None, out_dir: Path | None, broker_url: str | None,
         ...
         runtime: str, credentials_path: str | None) -> None:
...
@click.option("--password", default=None, hide_input=True, help="Login password; prefer AMP_PASSWORD.")
@click.option("--permission-mode", type=click.Choice(["acceptEdits", "bypassPermissions", "plan"]),
              default=DEFAULT_SHORTCUT_PERMISSION_MODE, show_default=True)
@click.option("--credentials-path", default=None, hidden=True)
